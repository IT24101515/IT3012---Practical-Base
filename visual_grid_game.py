...
class EnvironmentState:
	...
	def __init__(
		...
		num_opponents: int = 2,
		num_traps: int = 5,
		custom_walls: Optional[Set[Tuple[int, int]]] = None,
	) -> None:
		self.width = width
		self.height = height
		...
		opp_positions = self._generate_unique_positions(
			count=num_opponents,
			excluded=self.walls | self.food_positions | {(0, 0)},
		)
		self.opponents: List[List[int]] = [list(pos) for pos in opp_positions]

		# Add toxic traps avoiding starting position, walls, food and opponents
		excluded_for_traps = self.walls | self.food_positions | {tuple(op) for op in self.opponents} | {(0, 0)}
		self.toxic_traps: Set[Tuple[int, int]] = self._generate_unique_positions(
			count=num_traps,
			excluded=excluded_for_traps,
		)
	...
	def _generate_unique_positions(self, count: int, excluded: Set[Tuple[int, int]]) -> Set[Tuple[int, int]]:
		...
		while len(positions) < count and attempts < max_attempts:
			...
			pos = (fx, fy)
			if pos not in excluded and pos not in positions:
				positions.add(pos)
			attempts += 1
		return positions
...
class VisualGridHuntGame:
	...
	def get_percept(self) -> Dict[str, Any]:
		"""Perception Subsystem - Returns sensor data including 'smells_toxin'."""
		...
		return {
			...
			'smells_toxin': current_pos_tuple in self.state.toxic_traps,
	...
	def execute_action(self, action: str) -> None:
		...
		# Toxic trap interaction penalty
...
class GridGameGUI:
	...
	def draw_grid(self) -> None:
		"""Renders grid entities including purple toxic trap shapes on the canvas."""
		...
		# Render Toxic Traps (Custom Purple Shapes)
